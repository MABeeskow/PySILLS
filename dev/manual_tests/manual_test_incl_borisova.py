...
# Name:		manual_test_incl_borisova.py
...
"""
Module: manual_test_incl_borisova.py
...
def run_manual_test(show_full_df=False):
    ...
    for fname, setup_info in files_smpl_setup.items():
        reference_concentration_incl_t = reference_concentration_incl_t
        rho_mat = 2.65
        rho_incl = 0.8
        radius_mat = 24.5
        radius_incl = 22.5
        ...
        df_intensity_mix_t = df_intensities_mix[ref_isotope_t]
        df_intensity_mat_t = df_intensities_mat[ref_isotope_t]
        df_intensities_incl = df_intensities_mix - df_intensity_mix_t*(df_intensities_mat/df_intensity_mat_t)

        k = dri.calculate_integrated_intensity_borisova(
            intensities_incl=df_intensities_incl, intensity_mix_t=df_intensity_mix_t)

        df_concentrations_mat_t = df_concentrations_mat[ref_isotope_t]
        df_sensitivity_drift_t = df_sensitivity_drift/df_sensitivity_drift[ref_isotope_t]

        ## inclusion concentration
        df_concentrations_incl = SA(reference_isotope=ref_isotope_t).calculate_inclusion_concentration_by_borisova(
            concentrations_mat_t=df_concentrations_mat_t, rho_mat=rho_mat, rho_incl=rho_incl, radius_mat=radius_mat,
            radius_incl=radius_incl, sensitivities=df_sensitivity_drift_t, k=k)

        ## mixed concentration ratio a
        a = (df_intensity_mix_t/df_intensities_mix[ref_isotope])/df_sensitivity_drift[ref_isotope_t]
        df_concentrations_mat_is = df_concentrations_mat[ref_isotope]
        df_concentrations_incl_is = df_concentrations_incl[ref_isotope]

        ## mass fraction x
        x = (df_concentrations_mat_t - a*df_concentrations_mat_is)/(df_concentrations_mat_t - a*(
                df_concentrations_mat_is - df_concentrations_incl_is))

        concentration_mix_is = (1 - x)*df_concentrations_mat[ref_isotope] + x*df_concentrations_incl_is
        ...
        i_incl_ratios = dri.compute_intensity_ratios(intensities=df_intensities_incl, reference_isotope=ref_isotope)
        ...
        df_intensities_incl = df_intensities_incl.clip(lower=0.0)
